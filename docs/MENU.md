...
`ui volume <path>`, `ui advanced <path>`, `ui gpu-effects <path>`, `ui gpu-scenery <path>` and `ui gpu-lighting <path>` capture those pages; to capture the Modern look run with a window and `LBA2_GPU_CAPTURE=<file.bmp> LBA2_CAPTURE_MENU=1`, which keeps the last presented frame that had the menu up.
