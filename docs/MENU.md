...
Every page drawn through `DrawGameMenu` / `DoGameMenu` (main menu, Options and its sub-pages, the resolution list, dialogue choice lists) has two looks, chosen by `MenuStyle` (Display page row, `gfx_menustyle`, lba2.cfg):

| Value | Look |
|-------|------|
| 0 Classic | The 1997 plasma bars and bitmap font, drawn into the frame. |
| 1 Modern | Floating cards drawn over the frame in true colour at the window's resolution: rounded translucent cards with drop shadows, the cursor row enlarged with a pulsing blue glow and sparkles, `Label: value` selector rows split into a label and a value between chevrons, volume and detail rows as level bars. Text is Nunito Bold (SIL OFL 1.1). |
| 2 Auto (default) | Modern with the GPU renderer, Classic otherwise, so the default game looks as it always did. |

The save and load screens (`ChoosePlayerName`) follow the same style: a dimmed backdrop, the page title between sparkles, the scene preview as a framed true-colour picture (a sparkle card for the new-game slot), key hints (Enter confirm, Esc back, Del delete) in pills beside it, the slots as cards with scroll arrows beside the list, a golden card with a blinking caret while a name is typed, and the delete confirmation showing the save's name and picture above its buttons. `DrawOneString`, `DrawSingleString`, `DrawScreenSave` and `DrawCadreNewGame` record into the page state instead of drawing while that screen is up; elsewhere (the resolution revert dialog, the saved message) they draw as before.

The inventory (`INVENT.CPP`) and the dialogue box (`MESSAGE.CPP`) follow it too. The inventory keeps the classic dark slot wells under open overlay frames (the 3D objects are drawn in the frame beneath, so the overlay only darkens the panel between slots), marks the equipped weapon amber and the cursor slot with a pulsing blue glow, and shows counts, level bars and the object name as overlay text. The dialogue box becomes a translucent card with a bar in the speaker's colour; the engine still lays out lines and pages and types them out, but each pushed character lands in an overlay line drawn in Nunito, and the next-page arrow is an animated chevron. The holomap and island plan show their location name in a centred strip card and the target reticule as glowing overlay corners (same blink), and the found-object cinematic frames the rotating item in a golden card with a pulsing glow and sparkles circling it. The "Game saved" message is a toast card with a success-green edge and a check mark, and the credits draw their scrolling text as overlay text (gold headings between sparkles, white sections, light-blue names) with a soft fade at the top and bottom edges. The keyboard and game controller configuration screens (`CONFIG.CPP`) are rebuilt as overlay cards from their state on every classic redraw: a dimmed backdrop, the table panel with accent column titles, striped rows with the selected row in blue, key bindings as keycaps (the selected one glowing blue, amber and pulsing while waiting for a key), Default / Restore / Accept as buttons, and a status bar whose edge turns red on a conflicting key. The behaviour menu (`COMPORTE.CPP`, Normal / Sporty / Aggressive / Discreet and the other sets) becomes a dark glass panel: open tile cards around the turning Twinsens (the selected one glowing blue and pulsing), the behaviour name in a card with chevrons when Up/Down lead to another set, and a status card with gradient life and magic bars, the coins and keys as overlay numbers over the classic icons. With the GPU renderer the panel's box is untagged when it opens (`GpuObj_UntagBox`), so the scene's GPU colours no longer show through it. The overlay has layers (page, dialogue, menu) so a choice menu over a dialogue does not erase it.

Headless runs always draw Classic, so `ui` captures and their goldens are unaffected. Rows keep the classic centres and height, so mouse hit-testing and caller layouts are the same in both looks; in Modern, Log keeps only the page backdrop.

The cards come from the UI overlay ([LIB386/SYSTEM/UIOVERLAY.CPP](../LIB386/SYSTEM/UIOVERLAY.CPP)): a retained list of shapes and text in frame coordinates that `WINDOW.CPP` draws after the frame (and the GPU composite) on every present, until `DoGameMenu` returns and clears it. While it is up the overlay draws the mouse pointer and the software cursor sprite is hidden. The font is a signed distance field atlas generated by `scripts/dev/build-menu-font.py` into `LIB386/SYSTEM/FONTS/NUNITO_SDF.H` (license beside it), rasterized to the on-screen size so text stays sharp from 640x480 to 4K.

`ui volume <path>`, `ui advanced <path>`, `ui gpu-effects <path>`, `ui gpu-scenery <path>`, `ui gpu-lighting <path>` and `ui camera <path>` capture those pages; to capture the Modern look run with a window and `LBA2_GPU_CAPTURE=<file.bmp> LBA2_CAPTURE_MENU=1`, which keeps the last presented frame that had the menu up.
...
│   │   ├── Camera [custom localized label, community addition]
│   │   │   ├── Back (26)
│   │   │   ├── Camera (outdoors): < Free/Classic > [selector, FollowCamera; free is the default under the GPU renderer]
│   │   │   ├── Mouse camera: < Off/Right button/Always > [selector, MouseCamera + MouseCameraDrag]
│   │   │   ├── Mouse sensitivity: < 1–10 > [selector, MouseSensitivityX/Y together]
│   │   │   ├── Stick sensitivity: < 1–10 > [selector, GamepadCameraSensX/Y together]
│   │   │   ├── Invert vertical: < Off/On > [selector, MouseInvertY + GamepadCameraInvertY]
│   │   │   └── Rotation: < Free/Re-centers > [selector, cam_hold_angle]
...
│   │   │   │   └── Grass: < On/Off > [selector, gfx_grass]
