...
`ui volume <path>`, `ui advanced <path>` and `ui gpu-effects <path>` capture those pages; to capture the Modern look run with a window and `LBA2_GPU_CAPTURE=<file.bmp> LBA2_CAPTURE_MENU=1`, which keeps the last presented frame that had the menu up.
