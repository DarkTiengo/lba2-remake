...
| GpuWater | int | 0–1 | 1 | GPU renderer modern procedural water on every exterior sea, including shore foam and contact splashes. Console `gfx_water`. See [GPU_RENDERER.md](GPU_RENDERER.md) |
...
| GpuViewDistance | int | 0–2 | 1 | GPU renderer: view distance outdoors. 0 classic (3 × 3 cubes, the island's fog), 1 far (5 × 5 cubes, fog 1.6 times farther), 2 very far (7 × 7, 2.4 times). Console `gfx_viewdistance`. The classic renderer keeps the original |
| GpuAutoTuned | int | 0–1 | 0 | GPU renderer: the effects were adjusted to this machine. At 0 the first exterior drawn with the GPU renderer turns every effect on, measures the frame time and switches off, in order, very far view, ray tracing, far view, smooth terrain, soft shadows, modern water and global light until a frame fits 17.5 ms; then it saves 1. Console `gfx_autotune`, Display > Auto-adjust |
...
| GpuFastTerrain | int | 0–1 | 1 | GPU renderer: the software terrain fill is flat where the GPU covers it, since the compositor replaces its colour. Off restores the full textured fill (what the software frame looks like on its own). Console `gfx_fastterrain` |
| GpuGrass | int | 0–1 | 1 | GPU renderer: blades of grass swaying in the wind on the ground the island marks as grass (its footstep sound) where the texture is green; drawn only. Console `gfx_grass` |
...
| GpuRayTrace | int | 0–1 | 1 | GPU renderer: shadows ray traced outdoors through every body and the terrain, from the sun (with `GpuSunlight`) and the lights. Console `gfx_raytrace`. See [GPU_RENDERER.md](GPU_RENDERER.md) |
