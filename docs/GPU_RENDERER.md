...
**Lamps on models.** With `gfx_lamps` on (Display > GPU effects > Lamps on models, `GpuLamps`), a body whose unshaded surfaces (solid polygons, spheres) use a pale, bright, coloured palette entry, the way the models paint a street lamp's globe, casts a steady light in that colour from their centre (`T_GPUOBJ_DRAW::HasGlow`), with a larger radius in exterior scenes, whose world scale is far larger than a room's. Saturated but darker yellows (a hero's golden hair) are excluded. Only lamps modelled that way light up: in Citadel Island's square only one of the lamp posts is captured this way; the others are not yet understood.
