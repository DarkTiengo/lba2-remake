...
> **Display page (this fork).** The Display submenu is one page of selector rows (`MENU_TYPE_DISPLAY_SELECTOR` in `GAMEMENU.CPP`): Resolution, Vsync, Renderer and Display mode. Left/right or the mouse wheel change a value; toggles apply at once, the resolution applies on Enter and shows "(Enter)" until then. The list offers every ladder rung up to 3840x2160 plus the panel's native size, including sizes above the display, which supersample (the frame is scaled down filtered). Choices of 2160 lines and more render the software frame at half size; see [GFX_OPTIONS.md](GFX_OPTIONS.md).
